 ...
 const TaskDetailModal = ({ task, workspace, onClose, onUpdateTask, onConfirmDelete, onStartTask, onOpenEdit, onCompleteTask, onReopenTask, isArchived }) => { 
     ...
     // --- UPDATED: Connect Reopen Button ---
     const handleReopenClick = () => { if(onReopenTask) { onReopenTask(task); onClose(); }};
     const handleStartClick = () => { if(onStartTask) { onStartTask(task); onClose(); }};
     // --- NEW: Connect Complete Button ---
     const handleCompleteClick = () => { if(onCompleteTask) { onCompleteTask(task); onClose(); }};
 ...
 // --- ADDED isArchived PROP ---
 const KanbanTask = ({ task, onTaskClick, onConfirmDelete, isArchived }) => {
     // --- MODIFIED: Disable sorting if archived ---
     const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: task._id, disabled: isArchived });
     const style = { 
 ...
 const KanbanColumn = ({ column, tasks, onAddTaskClick, onTaskClick, onConfirmDeleteTask, isOver, isArchived }) => {
     ...
     return (
         <div className={`bg-slate-800 p-3 rounded-md w-72 flex-shrink-0 border border-slate-700 flex flex-col transition-colors duration-200 ease-in-out ${isOver ? 'bg-indigo-900/40 border-indigo-600 ring-1 ring-indigo-600' : ''}`}>
             ...
             {!isArchived && column.name.toLowerCase().includes('to do') && (
                  <button
 ...
 const DocumentEditorModal = ({ taskId, taskTitle, onClose, isReadOnly }) => {
     ...
     useEffect(() => {
         ...
         window.tinymce.init({
             ...
             toolbar: isReadOnly ? false : 'undo redo | blocks | bold italic | alignleft aligncenter alignright | bullist numlist outdent indent | link image | code | help', // --- MODIFIED: Hide toolbar if read-only
             menubar: false,
             statusbar: false,
             autoresize_bottom_margin: 20,
             height: "100%",
             readonly: isReadOnly, // --- MODIFIED: Use prop here ---
              init_instance_callback: (editor) => { 
                  ...
                  editor.on('change keyup', () => {
                      // --- MODIFIED: Don't save if read-only ---
     ...
     }, [firebaseReady, isTinymceLoaded, db, userId, taskId, docPath, taskTitle, isReadOnly]); // --- ADDED isReadOnly dependency
                 ...
                 <div className="flex justify-between items-center p-4 border-b border-slate-700 flex-shrink-0">
                     ...
                     <div className='flex items-center space-x-2'>
                         <span className={`text-xs px-2 py-0.5 rounded-full ${ saveStatus === 'Saved' ? 'bg-green-700 text-green-100' : saveStatus.startsWith('Saving') ? 'bg-yellow-700 text-yellow-100 animate-pulse' : saveStatus.startsWith('Error') ? 'bg-red-700 text-red-100' : 'bg-slate-600 text-slate-200' }`}>
                            {isReadOnly ? 'Read-Only' : saveStatus}
                         </span>
 ...
 const WorkspacePage = () => {
     ...
     function handleDragEnd(event) {
         ...
         // --- NEW: Workflow Rule Enforcement ---
         const sourceColumnName = getColumnName(sourceColumnId);
         const destColumnName = getColumnName(destinationColumnId);
 
         if (sourceColumnName && destColumnName) {
             // Block moving from "In Progress" back to "To Do"
             if (sourceColumnName.includes('in progress') && destColumnName.includes('to do')) {
                 console.warn("WORKFLOW BLOCK: Cannot move from 'In Progress' to 'To Do'.");
                 return; // Cancel the move
             }
             // Block moving from "Done" back to "To Do"
             if (sourceColumnName.includes('done') && destColumnName.includes('to do')) {
                 console.warn("WORKFLOW BLOCK: Cannot move from 'Done' to 'To Do'.");
                 return; // Cancel the move
             }
         }
         // --- END NEW RULES ---
     ...
     const handleOpenEditor = (task) => {
         ...
         // --- MODIFICATION: Only auto-move if NOT archived ---
         if (!isArchived && sourceColumn.name.toLowerCase().includes('to do')) {
     ...
     // --- NEW: Handler for Complete Task button ---
     const handleCompleteClick = (task) => {
         if (!task || !task._id) {
             console.error("Task data is missing for completion.");
             return;
         }
         const sourceBoard = boards.find(b => b._id === task.board);
         if (!sourceBoard) {
             console.error("Cannot find board for this task.");
             return;
         }
         const destColumn = findColumnByName(sourceBoard, 'done');
         if (destColumn) {
             console.log(`Task '${task.title}' is being moved to 'Done'.`);
             // Move task to the top (index 0) of the "Done" column
             moveTaskAndUpdateState(task, destColumn._id, 0);
         } else {
             console.warn("Could not find a 'Done' column to move task to.");
             alert("Could not find 'Done' column.");
         }
     };
 
     // --- NEW: Handle Reopen Task Click ---
     const handleReopenClick = (task) => {
         if (!task || !task._id) {
             console.error("Task data is missing for reopening.");
             return;
         }
         const sourceBoard = boards.find(b => b._id === task.board);
         if (!sourceBoard) {
         ...
         const destColumn = findColumnByName(sourceBoard, 'in progress');
         if (destColumn) {
             console.log(`Task '${task.title}' is being reopened. Moving to 'In Progress'.`);
             // Move task to the top (index 0) of the "In Progress" column
             moveTaskAndUpdateState(task, destColumn._id, 0);
         } else {
             console.warn("Could not find an 'In Progress' column to move task to.");
             alert("Could not find 'In Progress' column.");
         }
     };
         ...
         <div className="min-h-screen bg-slate-900 text-white p-8 font-inter">
              <header className="mb-8 flex flex-wrap justify-between items-start gap-y-4">
                   ...
                   <div className="flex flex-col items-end gap-y-2"> {/* CHANGED: Made this a flex-col aligned to the end */}
                       ...
                       {/* --- MODIFIED: Ownership check REMOVED for testing. --- */}
                       {/* --- WARNING: This allows ANY logged-in user to SEE the delete button for this workspace. --- */}
                       {/* --- Original check was: {!loading && workspace && currentUser && workspace.owner._id === currentUser.user._id && ( --- */}
             ...
             {!loading && !error && ( 
                 <DndContext
                     ...
                     disabled={isArchived} // --- MODIFIED: Disable DND if archived ---
                  >
                     <div>
                             ...
                             !loading && !error && ( // Keep this condition simple
                                <div className="text-center py-10 px-6 bg-slate-800 rounded-lg max-w-md mx-auto border border-slate-700">
                                    ...
                                    <p className="text-slate-400 mb-6">Create your first board to start organizing tasks.</p>
                                     {/* --- MODIFIED: Hide create form if archived --- */}
                                    {!isArchived && (
                     ...
                     <DragOverlay>{ activeTask ? ( <KanbanTask task={activeTask} onTaskClick={()=>{}} onConfirmDelete={()=>{}} isArchived={isArchived} /> ) : null }</DragOverlay>
                 </DndContext>
             )}
             ...
             {selectedTask && workspace && (
                 <TaskDetailModal
                     ...
                     onCompleteTask={handleCompleteClick} // --- ADDED
                     onReopenTask={handleReopenClick}   // --- ADDED
                     isArchived={isArchived} // --- Pass prop down ---
                 />
             )}
             ...
             {editingTask && (
                  <DocumentEditorModal 
                     ...
                     isReadOnly={isArchived} // --- Pass prop down ---
                  />
             )}
